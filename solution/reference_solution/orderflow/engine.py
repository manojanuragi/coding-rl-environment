...
def price_order(order: Order) -> PricedOrder:
    ...
    # Discounts apply to merchandise only -- shipping is never part of the
    # discountable amount (docs/PRICING_RULES.md, "Order of operations", #2).
    discounted_subtotal = apply_discounts(subtotal, order.discounts)
    discount_amount = _round_currency(subtotal - discounted_subtotal)

    tax_rate = _blended_tax_rate(order, subtotal)
    tax = _round_currency(discounted_subtotal * tax_rate)

    total = _round_currency(discounted_subtotal + shipping_cost + tax)
